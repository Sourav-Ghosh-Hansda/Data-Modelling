...
myd.hclust <- hclust(distance)
myd.hclust
plot(myd.hclust)
rect.hclust(myd.hclust, k = 2, border = "brown")

# Hierarchical agglomerative clustering using "average" linkage 
mydata.hclust<-hclust(distance,method="average")
plot(mydata.hclust)
#rect.hclust(mydata.hclust, k = 3, border = "brown")


# Cluster membership
member <- cutree(myd.hclust,2)
table(member)

# Characterizing clusters 
aggregate(nor,list(member),mean)
aggregate(new_data_clean[,-c(1,1)],list(member),mean)
